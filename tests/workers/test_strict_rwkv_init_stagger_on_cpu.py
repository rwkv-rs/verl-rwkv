...
from verl.workers.engine.rwkv_lm.initialization import (
    acquire_init_slot,
    init_delay_seconds,
    init_slot,
    release_init_slot,
...
def test_strict_rwkv_init_delay_staggers_ranks_without_changing_rank_zero():
    assert init_delay_seconds(0, "30") == 0
    assert init_delay_seconds(7, "30") == 210
    assert init_delay_seconds(3, None) == 0


@pytest.mark.parametrize("value", ["-1", "301", "invalid"])
def test_strict_rwkv_init_delay_rejects_unsafe_values(value):
    with pytest.raises(RuntimeError):
        init_delay_seconds(1, value)


def test_strict_rwkv_init_slot_is_released_after_failure(tmp_path):
    with pytest.raises(RuntimeError, match="boom"):
        with init_slot("1", str(tmp_path)):
            raise RuntimeError("boom")

    handle = acquire_init_slot("1", str(tmp_path))
